...
const { calculateInactiveMonths, calculateMonthsAsCustomer  } = require("./utils/helper");
const { GoogleGenerativeAI } = require("@google/generative-ai");
const app = express();
app.use(cors());
app.use(bp.json());

const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY || "AIzaSyDuGJM-eeTKZXKaZ8tmqYt_C6OuSE8ktuc");
...
app.get("/predict-customer/:id", async (req, res) => {
  try {
    ...
    const transactions = await db.collection("transactions").find({ customer_id: customer._id }).toArray();
    ...
    const customerData = {
      ...
      Total_Transaction_Amount: transactions.reduce((sum, txn) => sum + txn.amount, 0),
      Inactive_Months_12_Months: calculateInactiveMonths(transactions),
      Transaction_Count_Change_Q4_Q1: transactions.length > 0 ? ((transactions.length / 12) * 4) : 0,
      ...
      Transaction_Amount_Change_Q4_Q1: transactions.length > 0 ? ((transactions.reduce((sum, txn) => sum + txn.amount, 0) / 12) * 4) : 0,
    ...
    const fastApiResponse = await axios.post("http://localhost:8000/predict", { customers: [customerData] });
...
app.get("/get-all-users", async (req, res) => {
  try {
    ...
    // Fetch users
    const users = await collection.find({}, { projection: { _id: 1, name: 1, email: 1 } }).toArray();
...
app.get("/predict-customers", async (req, res) => {
  try {
      ...
      const fastApiResponse = await axios.post("http://localhost:8000/predict", { customers: customersData });
...
app.post("/predict-single-customer", async (req, res) => { 
  try {
    ...
    const customerDataUpdated = {
      ...
      Transaction_Count_Change_Q4_Q1: customer.Transaction_Count_Change_Q4_Q1 || 0,
      Total_Products_Used: customer.total_products_used || 0,
      Average_Credit_Utilization: customer.credit_utilization || 0,
      Customer_Contacts_12_Months: customer.customer_contacts || 0,
      Transaction_Amount_Change_Q4_Q1: customer.Transaction_Amount_Change_Q4_Q1 || 0,
    ...
    // Call FastAPI for prediction
    const fastApiResponse = await axios.post("http://localhost:8000/predict", { customers: [customerDataUpdated] });
    const prediction = fastApiResponse.data.predictions[0];
...
app.get("/get-feedbacks/:id", async (req, res) => {
  ...
  try {
    ...
    const transactions = await transactionsCollection.find({ customer_id: new ObjectId(customerId), feedback: { $exists: true, $ne: "" } }).toArray();
    const improvements = await improvementsCollection.find({ customerId: customerId }).toArray();
    if (!transactions.length) {
      return res.status(405).json({ message: "No feedbacks found for this customer." });
    }
    ...
    res.json({ feedbacks, improvements: improvements.length ? improvements : null });
  } catch (error) {
...
app.get("/generate-suggestions/:customerId", async (req, res) => {
  ...
  const transactionsCollection = db.collection("transactions");
  let feedbacks = await transactionsCollection.find({ customer_id: new ObjectId(customerId), feedback: { $exists: true, $ne: "" } }).toArray();
  if (!feedbacks.length) {
...
app.post("/send-improvements", async (req, res) => {
  try {
      ...
      if (!customerId || !Array.isArray(selectedImprovements) || selectedImprovements.length === 0) {
