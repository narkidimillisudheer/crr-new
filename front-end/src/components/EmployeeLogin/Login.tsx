...
const Login: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      const res = await axios.post("http://localhost:5000/employee-login", data);
      ...
      <main className="flex flex-col items-center justify-center w-full flex-1 px-20 text-center">
        <div className="flex w-2/3 max-w-4xl bg-white rounded-[10px] shadow-lg shadow-gray-500">
          <div className="w-1/2 p-2">
            ...
            <form action="" onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="flex flex-col items-center">
                ...
                <div>
                  <button
                    className="w-36 py-2.5 my-4 bg-primary hover:bg-[#33384a]
          ...
          <div className="w-1/2 bg-gradient-to-r from-[#00040f] to-[#33384a] text-white rounded-r-[10px] py-48 px-14">
            <h2 className="text-3xl font-extrabold mb-4">Employee login</h2>
            <p>Bank employee login form here</p>
          </div>
        </div>
      </main>
      <Snackbar
